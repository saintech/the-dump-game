...
pub fn update(world: &mut game::World) {
    ...
    let player_on_stairs = world
        .map_obj_iter()
        .any(|(_, sym, map_obj, ..)| ((sym.x, sym.y) == player_pos) && (map_obj.name == "спуск"));
...
fn pick_item_up(object_id: u32, world: &mut game::World) {
    ...
    if inventory_len >= 35 {
        world.add_log(
            cfg::COLOR_DARK_RED,
            format!("Ваш инвентарь заполнен, невозможно взять {}.", name),
        );
    } else {
        world.add_log(cfg::COLOR_GREEN, format!("Вы подобрали {}!", name));
...
fn next_level(world: &mut game::World) {
    ...
    world.add_log(
        cfg::COLOR_GREEN,
        "Вам нужно немного отдохнуть и восстановить силы.",
    );
    let heal_hp = world.max_hp(world.player.id) / 2;
    heal(world.player.id, heal_hp, world);
    world.add_log(
        cfg::COLOR_ORANGE,
        "После редкой минуты покоя вы спускаетесь все глубже \
