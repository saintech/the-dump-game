...
impl ANSICyrString {
    pub fn new(s: &str) -> Self {
        ...
        ANSICyrString(mapped_chars.collect::<Vec<_>>())
    }
}

impl AsRef<[u8]> for ANSICyrString {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl TcodString for ANSICyrString {
...
fn render_map(world: &game::World, con: &mut impl console::Console) {
    ...
    for i in 0..map.len() {
        ...
        let (fg, bg, glyph) = match (visible, wall, wall_bottom, ground_sprite) {
            ...
            (false, true, false, _) => (cfg::COLOR_DARK_WALL, cfg::COLOR_DARK_WALL_BG, '\u{130}'),
            (false, true, true, _) => (cfg::COLOR_DARK_WALL, cfg::COLOR_DARK_WALL_BG, '\u{131}'),
            (false, false, _, false) => (cfg::COLOR_DARK_GROUND, cfg::COLOR_DARK_GROUND_BG, ' '),
            (false, false, _, true) => {
                (cfg::COLOR_DARK_GROUND, cfg::COLOR_DARK_GROUND_BG, '\u{132}')
            }
            // inside fov:
            (true, true, false, _) => (cfg::COLOR_LIGHT_WALL, cfg::COLOR_LIGHT_WALL_BG, '\u{130}'),
            (true, true, true, _) => (cfg::COLOR_LIGHT_WALL, cfg::COLOR_LIGHT_WALL_BG, '\u{131}'),
            (true, false, _, false) => (cfg::COLOR_LIGHT_GROUND, ground_bg, ' '),
            (true, false, _, true) => (cfg::COLOR_LIGHT_GROUND, ground_bg, '\u{132}'),
        };
        if map[i].explored {
...
fn render_main_menu_bg(con: &mut impl console::Console) {
    ...
    con.set_default_foreground(cfg::COLOR_SKY);
    con.print_ex(
        cfg::SCREEN_WIDTH / 2,
        cfg::SCREEN_HEIGHT - 2,
...
fn render_dialogs(world: &game::World, destination_console: &mut impl console::Console) {
    ...
    } in &world.dialogs
    {
        ...
        window.print_rect(1, 1, width - 1, height, ANSICyrString::new(header));
        ...
        for (index, option_text) in options.iter().enumerate() {
            ...
            window.print(
                1,
                header_height + 2 + index as i32,
                ANSICyrString::new(&text),
            );
        }
