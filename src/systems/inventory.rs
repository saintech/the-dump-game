    ...
    if is_opening_inventory {
        let (dialog_kind, menu_title) = match world.player.action {
            PlayerAction::OpenInventory => (
                ...
                "Чтобы использовать, нажмите на соответствующую клавишу, или Esc для отмены.",
            ),
            PlayerAction::DropItem => (
                DialogKind::DropItem,
                "Чтобы выбросить, нажмите на соответствующую клавишу, или Esc для отмены.'",
            ),
            _ => unreachable!(),
        };
    ...
    let mut options: Vec<_> = world
        ...
        .map(|(.., map_obj, _, eqp, ammo)| {
            ...
            let equipped_postfix = eqp
                ...
                .map_or(String::new(), |eqp| format!(" ({})", eqp.slot));
            map_obj.name.clone() + &count_postfix + &equipped_postfix
        })
        .collect();
    if options.is_empty() {
        options.push(String::from("Инвентарь пуст."));
    }
...
fn use_item(inventory_id: u32, world: &mut game::World, by_targeting: bool) {
    ...
    if let Some((.., item, _, _)) = world.get_item(inventory_id) {
        ...
        match on_use(inventory_id, world, by_targeting) {
            ...
            UseResult::Cancelled => world.add_log(cfg::COLOR_LIGHTEST_GREY, "Отменено"),
        ...
        world.add_log(
            ...
            format!("{} не может быть использован.", name),
        );
    }
}
...
fn use_medkit(_inventory_id: u32, world: &mut game::World, _by_targeting: bool) -> UseResult {
    ...
    if world.player_char().hp == world.max_hp(world.player.id) {
        world.add_log(cfg::COLOR_ORANGE, "Вы и так полностью здоровы.");
        return UseResult::Cancelled;
    }
    world.add_log(cfg::COLOR_GREEN, "Ваши раны начинают выглядеть лучше!");
...
fn throw_brick(_inventory_id: u32, world: &mut game::World, by_targeting: bool) -> UseResult {
    if !by_targeting {
        ...
        world.add_log(
            cfg::COLOR_DARK_SKY,
            "Левый клик на цели, чтобы бросить в нее кирпич, или правый для отмены.",
        );
        UseResult::NeedTargeting
    } else {
        ...
        if let Some(monster_id) = monster_id {
            ...
            world.add_log(
                cfg::COLOR_LIGHTEST_GREY,
                format!(
                    "{} выглядит ошалелым и начинает слоняться вокруг.",
                    monster_name
                ),
            );
            UseResult::UsedUp
        } else {
            world.add_log(cfg::COLOR_DARK_SKY, "Нет цели в досягаемости броска.");
            UseResult::Cancelled
        }
    }
}
...
fn throw_blasting_cartridge(
    ...
    if !by_targeting {
        world.add_log(
            cfg::COLOR_DARK_SKY,
            "Левый клик чтобы бросить туда заряд, или правый для отмены.",
        );
        UseResult::NeedTargeting
    } else {
        ...
        world.add_log(
            cfg::COLOR_ORANGE,
            format!(
                "Подрывной заряд взрывается, сокрушая все в радиусе {} клеток!",
                cfg::BLASTING_RADIUS
            ),
        );
        ...
        for target_id in targets {
            ...
            world.add_log(
                cfg::COLOR_LIGHTEST_GREY,
                format!(
                    "{} получает урон в размере {} очков здоровья.",
                    target_name,
                    cfg::BLASTING_DAMAGE
                ),
            );
...
fn toggle_equipment(inventory_id: u32, world: &mut game::World, _by_targeting: bool) -> UseResult {
    ...
    if equipment.equipped {
        ...
        world.add_log(
            cfg::COLOR_ORANGE,
            format!("Похоже, этот боеприпас сюда совсем не подходит."),
        );
    } else {
...
fn dequip(id: u32, world: &mut game::World) {
    ...
    if let Some(equipment) = world.get_item_mut(id).unwrap().3 {
        if equipment.equipped {
            ...
            world.add_log(
                cfg::COLOR_DARK_SKY,
                format!("{} больше не используется в {}.", name, slot),
            );
        }
    } else {
        world.add_log(
            cfg::COLOR_ORANGE,
            format!("Нельзя снять {}, т.к. это не снаряжение.", name),
        );
    }
}
...
fn drop_item(inventory_id: u32, world: &mut game::World) {
    ...
    world.add_log(cfg::COLOR_DARK_SKY, format!("Вы выбросили {}.", name));
}
